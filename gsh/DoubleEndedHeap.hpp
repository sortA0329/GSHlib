...
#include <memory>
namespace gsh {
template<class T, class Comp = Less, class Alloc = std::allocator<T>> class DoubleEndedHeap {
  Vec<T, Alloc> data;
...
public:
  ...
  constexpr const_reference top() const noexcept { return data[mx]; }
  ...
  constexpr void pop() noexcept(nothrow_op) { pop_max(); }
  ...
  constexpr void replace(const T& x) noexcept(nothrow_op && std::is_nothrow_copy_assignable_v<T>) { replace_max(x); }
  constexpr void replace(T&& x) noexcept(nothrow_op) { replace_max(std::move(x)); }
  ...
  constexpr void pushpop(const T& x) noexcept(nothrow_op && std::is_nothrow_copy_assignable_v<T>) { pushpop_max(x); }
  constexpr void pushpop(T&& x) noexcept(nothrow_op) { pushpop_max(std::move(x)); }
